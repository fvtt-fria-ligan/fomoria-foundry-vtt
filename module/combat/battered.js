...
const BATTERED_ROLL_CARD_TEMPLATE =
  "systems/fomoria/templates/chat/battered-roll-card.html";
...
export async function rollBattered(actor) {
  ...
  if (batteredRoll.total <= 2) {
    ...
    const roundsWord = pluralize("FO.Round", "FO.Rounds", roundsRoll.total);
    const hpRoll = await new Roll("1d4").evaluate();
    outcomeLines = [
      game.i18n.format("FO.BatteredUnconscious", {
    ...
    if (presenceRoll.total >= rageDR) {
      ...
      const hpRoll = await new Roll("1d4").evaluate();
      outcomeLines = [
        game.i18n.localize("FO.CyRageAverted") + ".",
        game.i18n.format("FO.BatteredUnconscious", {
          rounds: roundsRoll.total,
          roundsWord,
          hp: hpRoll.total,
      ...
      outcomeLines = [
        game.i18n.format("FO.BatteredCyRage", {
          hp: hpRoll.total,
        }),
      ];
    ...
    const hpRoll = await new Roll("1d4").evaluate();
    outcomeLines = [
      game.i18n.format("FO.BatteredCriticalInjury", {
        bodyPart,
        hp: hpRoll.total,
        rounds: roundsRoll.total,
        roundsWord,
    ...
    const hoursWord = pluralize("FO.Hour", "FO.Hours", hoursRoll.total);
    const drModifier = game.i18n.localize(hoursRoll.total == 1 ? "FO.BatteredHemorrhageOneHour" : "FO.BatteredHemorrhageTwoHours");
    outcomeLines = [
      game.i18n.format("FO.BatteredHemorrhage", {
    ...
    // death
    outcomeLines = [game.i18n.localize("FO.BatteredDead")];
  }
